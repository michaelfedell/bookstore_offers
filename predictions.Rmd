...
predictions <- test.full %>%
  select(id, avgNetOrder, sumQty, recency, frequency, tof, amount, maxNet,
...
optProfit <- max(cumsum(arrange(predictions, -predTargamt)$profit)); optProfit
```

This profit is obtained by marketing to the top x customers where x is calculated below

```{r}
optCustomers <- which.max(cumsum(arrange(predictions, -predTargamt)$profit)); optCustomers
```
...
list('SSEP' = SSEP, 
     ...
     'Optimal Profit' = optProfit,
     'Optiumum Target Customers' = optCustomers)
```
