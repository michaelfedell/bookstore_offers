...
```{r}
subset <- book.train.responders %>% select(logtargamt, amount, recency, frequency, tof) %>% mutate(af = amount * frequency, ap = amount / (tof + 0.0001))
...
best2 <- lm(logtargamt ~ amount + frequency + I(amount * frequency) +
              I(amount / (log(tof))), book.train.responders)
summary(best2)
```
