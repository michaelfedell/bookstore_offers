...
Amount, AvgCost and Frequency are the most non-significant predictor, so we first remove it.

```{r}
mod2_logistic = glm(Respond ~ TotalQty + Recency + Time, data = train_final, family = binomial)
...
```{r}
test_final = book.test.full[,c(1:8, 69)]
colnames(test_final) = c("ID", "AvgCost", "TotalQty", "logtargamt", "Recency", "Frequency", "Amount", "Time", "Respond")
...
predicted1 = predict(mod2_logistic, test_final, type = "response")
```

```{r}
SSE_test1 = sum((as.numeric(test_final$Respond) - predicted)^2); SSE_test1
SSE_test2 = sum((as.numeric(test_final$Respond) - predicted1)^2); SSE_test2
```
